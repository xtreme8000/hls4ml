...
from hls4ml.backends.fpga.fpga_types import HLSTypeConverter, APTypeConverter
...
class VivadoAcceleratorWriter(VivadoWriter):
    ...
    def write_axi_wrapper(self, model):
        ...
        hls_conv = APTypeConverter() #HLSTypeConverter(precision_converter=APTypeConverter())
        hls_conv.convert(inp_axi_t)
        hls_conv.convert(out_axi_t)
        ...
        for line in f.readlines():
            if 'MYPROJECT' in line:
                ...
                if self.vivado_accelerator_config.get_interface() == 'axi_stream':
                    newline += f'typedef {inp_axi_t.definition_cpp()} T_in;\n'
                    newline += f'typedef {out_axi_t.definition_cpp()} T_out;\n'
                    newline += (
                        ...
                        + indent
                        + '{{ return stream << "{{ data: " << in.data{str_in} << ", last: " << in.last{str_in} << " }}" << std::endl; }}\n'
                        + indent
                    ...
                    newline += (
                        ...
                        + indent
                        + '{{ return stream << "{{ data: " << out.data{str_out} << ", last: " << out.last{str_out} << " }}" << std::endl; }}\n'
                        + indent
                        + 'operator float() const {{return this->data;}}\n'
                        + indent
                        + 'operator double() const {{return this->data;}}\n'
                    ...
                    newline = newline.format(
                        str_in='.to_string()' if not (inp_axi_t == 'float' or inp_axi_t == 'double') else '',
                        str_out='.to_string()' if not (out_axi_t == 'float' or out_axi_t == 'double') else '',
                    )
                else:
                    newline += f'typedef {inp_axi_t.definition_cpp()} input_axi_t;\n'
                    newline += f'typedef {out_axi_t.definition_cpp()} output_axi_t;\n'
            else:
                newline = line
            fout.write(newline)
        f.close()
